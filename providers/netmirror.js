...
const BASE_HEADERS = {
  ...
  // Updated UA to match Kotlin (Pixel 5 / Android 13)
...
function bypass() {
  ...
  function attemptBypass(attempts) {
    ...
    // Kotlin Utils.kt: post("$mainUrl/tv/p.php")
...
function searchContent(query, platform) {
  ...
  // Map based on Kotlin logic: Disney/Hotstar use "hs" path logic
  ...
  return bypass().then(function(cookie) {
    ...
    // Endpoints updated to net52.cc structure
...
function getStreamingLinks(contentId, title, platform) {
  ...
  return bypass().then(function(cookie) {
    ...
    // Logic updated based on Kotlin Provider files
    if (platform.toLowerCase() === "primevideo") {
        playlistUrl = `${NETMIRROR_BASE}/pv/playlist.php`;
        reqReferer = `${NETMIRROR_BASE}/home`; // Matches PrimeVideoMirrorProvider.kt
    } 
    else if (platform.toLowerCase() === "disney") {
        playlistUrl = `${NETMIRROR_BASE}/mobile/hs/playlist.php`;
        reqReferer = `${NETMIRROR_BASE}/home`; // Matches HotStarMirrorProvider.kt
    } 
    else {
        // Netflix
        // Crucial Change: Kotlin now uses /mobile/playlist.php for Netflix too
        playlistUrl = `${NETMIRROR_BASE}/mobile/playlist.php`; 
        reqReferer = `${NETMIRROR_BASE}/`; // Matches NetflixMirrorProvider.kt
    }
    ...
    playlist.forEach((item) => {
      ...
      if (item.sources) {
        item.sources.forEach((source) => {
          ...
          // Kotlin Logic Fixes:
          // 1. Prime Video: replace("/tv/", "/")
          if (platform.toLowerCase() === "primevideo" && fullUrl.includes("/tv/")) {
          ...
          try {
              if (fullUrl.startsWith('//')) fullUrl = 'https:' + fullUrl;
                  ...
                  // Ensure we use NETMIRROR_BASE (net52.cc)
                  fullUrl = NETMIRROR_BASE + (fullUrl.startsWith('/') ? '' : '/') + fullUrl;
              }
          } catch(e) {
